...
class CoattentionBiLSTMModel():
    ...
    def contextual_preprocessing(self, debug=False):
        ...
        with tf.variable_scope("QD-ENCODE"):
            ...
            cell_bw = tf.nn.rnn_cell.LSTMCell(num_units=FLAGS.state_size)
            ((Q_fw, Q_bw), _) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, Q_embed, sequence_length=self.question_seq_placeholder, dtype=tf.float32)
            Q = tf.concat(2, [Q_fw, Q_bw])
            tf.get_variable_scope().reuse_variables()
            ((D_fw, D_bw), _) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, D_embed, sequence_length=self.document_seq_placeholder, dtype=tf.float32)
            D = tf.concat(2, [D_fw, D_bw])
        
        assert_shape(Q, "Q", [None, FLAGS.max_question_size, 2 * FLAGS.state_size])
        assert_shape(D, "D", [None, FLAGS.max_document_size, 2 * FLAGS.state_size])
        ...
        with tf.variable_scope("Q-TANH"):
            W_q = tf.get_variable("W_q", shape=(2 * FLAGS.state_size, 2 * FLAGS.state_size),
                                  dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
            b_q = tf.get_variable("b_q", shape=(2 * FLAGS.state_size),
    ...
    def coattention_encode(self, preprocessing, debug=False):
        ...
        max_document_size = FLAGS.max_summary_size if FLAGS.max_summary_size < FLAGS.max_document_size else FLAGS.max_document_size
        # Affinity matrix.
        L = tf.batch_matmul(Q, tf.transpose(D, [0, 2, 1]))
        assert_shape(L, "L", [None, FLAGS.max_question_size, max_document_size])

        # Normalize with respect to question/document.
        A_q = tf.nn.softmax(L)
        assert_shape(A_q, "A_q", [None, FLAGS.max_question_size, max_document_size])
        A_d = tf.nn.softmax(tf.transpose(L, perm=[0,2,1]))
        assert_shape(A_d, "A_d", [None, max_document_size, FLAGS.max_question_size])

        # Attention of the document w.r.t question.
        C_q = tf.batch_matmul(A_q, D)
        assert_shape(C_q, "C_q", [None, FLAGS.max_question_size, 2 * FLAGS.state_size])
        ...
        assert_shape(C_d, "C_d", [None, max_document_size, 4 * FLAGS.state_size])

        # Fusion of temporal information to the coattention context
        with tf.variable_scope("COATTENTION"):
            coatt = tf.concat(2, [D, C_d])
            assert_shape(coatt, "coatt", [None, max_document_size, 6 * FLAGS.state_size])
    ...
    def answer_pointer_decode(self, encode, debug=False):
        ...
        summary_start = encode[1]
        max_document_size = FLAGS.max_summary_size if FLAGS.max_summary_size < FLAGS.max_document_size else FLAGS.max_document_size
        assert_shape(H_r, "H_r", [None, max_document_size, 2 * FLAGS.state_size])
        ...
        with tf.variable_scope("answer_pointer_decode"):
            ...
            for k in range(2):
                ...
                VH_r = _3d_X_2d(H_r, V)
                assert_shape(VH_r, "VH_r", [None, max_document_size, FLAGS.state_size])
                ...
                F_k = tf.nn.tanh(VH_r + tf.tile(W_aH_ab_a, [1, max_document_size, 1]))
                # F_k = tf.transpose(F_k, perm=[0, 2, 1])
                assert_shape(F_k, "F_k", [None, max_document_size, FLAGS.state_size])
                
                v_tF_k = _3d_X_2d(F_k, v)
                v_tF_k = tf.transpose(_3d_X_2d(F_k, v), perm=[0,2,1])
                assert_shape(v_tF_k, "v_tF_k", [None, 1, max_document_size])

                beta_no_softmax = v_tF_k + tf.tile(c, [1, max_document_size])
                beta_k = tf.nn.softmax(beta_no_softmax)
                assert_shape(beta_k, "beta_k", [None, 1, max_document_size])
            ...
            assert_shape(beta, "beta", [None, 2, max_document_size])
    ...
    def cross_entropy_loss(self, decode, debug=False):
        ...
        # mask = tf.logical_and(tf.greater_equal(s, 0), tf.less(e, max_document_size))
